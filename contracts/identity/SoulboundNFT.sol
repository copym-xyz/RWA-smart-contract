...
contract SoulboundNFT is ERC721, ERC721URIStorage, AccessControl {
    ...
    bytes32 public constant CREDENTIAL_MANAGER_ROLE =
        keccak256("CREDENTIAL_MANAGER_ROLE");
    ...
    struct TokenHierarchy {
        ...
        mapping(bytes32 => bool) credentialValidity;
        bytes32 credentialsMerkleRoot;
    }
    ...
    // Add a mapping to find token by credential hash
    ...
    function verifyIdentity(
        ...
        hierarchy.chainDetails[currentChainId] = Strings.toHexString(
            uint256(uint160(entity)),
            20
        );
    ...
    function addChainIdentity(
        ...
        require(
            ownerOf(tokenId) == msg.sender ||
                hasRole(CREDENTIAL_MANAGER_ROLE, msg.sender),
            "Not authorized"
        );
    ...
    function issueCredential(
        ...
        require(
            ownerOf(issuerTokenId) == msg.sender ||
                hasRole(CREDENTIAL_MANAGER_ROLE, msg.sender),
    ...
    /**
 * @dev Add a credential hash to a token
 */
function addCredentialToToken(
    uint256 tokenId,
    bytes32 credentialHash,
    bool isValid
) external onlyRole(CREDENTIAL_MANAGER_ROLE) {
    ...
    TokenHierarchy storage hierarchy = tokenHierarchy[tokenId];
    hierarchy.credentialHashes.push(credentialHash);
    hierarchy.credentialValidity[credentialHash] = isValid;
    
    // Map credential hash to token ID for reverse lookup
    credentialHashToTokenId[credentialHash] = tokenId;
    
    emit CredentialAddedToToken(tokenId, credentialHash, isValid);
}

/**
 * @dev Update credential validity status
 */
function updateCredentialStatus(
    bytes32 credentialHash,
    bool isValid
) external onlyRole(CREDENTIAL_MANAGER_ROLE) {
    uint256 tokenId = credentialHashToTokenId[credentialHash];
    require(tokenId != 0, "Credential not found");
    
    TokenHierarchy storage hierarchy = tokenHierarchy[tokenId];
    hierarchy.credentialValidity[credentialHash] = isValid;
    
    emit CredentialStatusUpdated(tokenId, credentialHash, isValid);
}

/**
 * @dev Get all credential hashes for a token
 */
function getTokenCredentials(uint256 tokenId) external view returns (bytes32[] memory) {
    require(_exists(tokenId), "Token does not exist");
    return tokenHierarchy[tokenId].credentialHashes;
}

/**
 * @dev Check if a credential is valid for a token
 */
function isCredentialValid(uint256 tokenId, bytes32 credentialHash) external view returns (bool) {
    require(_exists(tokenId), "Token does not exist");
    return tokenHierarchy[tokenId].credentialValidity[credentialHash];
}

// Add new events
event CredentialAddedToToken(uint256 indexed tokenId, bytes32 credentialHash, bool isValid);
event CredentialStatusUpdated(uint256 indexed tokenId, bytes32 credentialHash, bool isValid);
}
